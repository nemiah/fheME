...
class CCRetoure implements iCustomContent {
	private $loggedIn = true;
	function __construct() {
		#if(!isset($_GET["benutzer"])){
		#	$_GET["benutzer"] = "Max";
		#	$_GET["password"] = sha1("Max");
			#$_POST["belegart"] = "R";
		...
		#registerClassPath("Kundenpreise", Util::getRootPath()."open3A/Kundenpreise/Kundenpreise.class.php");
		
		#if(Session::currentUser() == null)# AND !Users::login($_GET["benutzer"], $_GET["password"], "open3A"))
	...
	function getLabel(){
		return "Retoure-Erfassung";
	}
	
	function getCMSHTML() {
		
		/*if(filter_input(INPUT_GET, "page") == "login"){
			...
			$T->setSaveCustomerPage("Anmelden", "", false, "function(){ document.location.href='?CC=Retoure' }");
		...
		if(!$this->loggedIn){
			...
			$T->setSaveCustomerPage("Anmelden", "", false, "function(){ document.location.reload();/*href='?CC=Retoure'*/ }");
			
			return $T;
			
			#header("Location: ?CC=Retoure&page=login");
			#die();
	...
	public function getAuftrag($data){
		...
		$TPosten = new HTMLTable(3, "Retoure");
		...
		$IOK->onclick("CustomerPage.rme('handleRetoure', $('#posten').serialize(), function(transport){ $('#contentLeft h1').html('Auftrag'); $('.selected').removeClass('selected'); $('#contentLeft .content').html(transport); })");
		...
		$IA = new HTMLInput("action", "hidden", "handleRetoure");
	...
	public function getLieferscheine($data){
		...
		while($B = $AC->n()){
			...
			$T->addRowEvent("click", "$('.selected').removeClass('selected'); $(this).addClass('selected'); CustomerPage.rme('getAuftrag', {GRLBMID: ".$B->getID()."}, function(transport){ if(transport == 'TIMEOUT') { document.location.reload();/*href='?CC=Retoure&page=login';*/ return; } $('#contentLeft .content').html(transport); }, function(){}, 'POST');");
	...
	function handleRetoure($data){
		...
		die("<p class=\"confirm\" style=\"margin-top:10px;\">Retoure gespeichert!</p>");
	}
