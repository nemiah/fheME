...
class CCBeleg extends CCAuftrag implements iCustomContent {
	function __construct() {
		parent::__construct();
		$this->showZahlungsart = true;
		#$this->showPrices = false;
		
		$this->loadPlugin("open3A", "Auftraege");
	...
	function getCMSHTML() {
		...
		if(!$HID)
			return "<p>Der zugehörige Beleg konnte nicht gefunden werden.</p>";
			
		$AC = anyC::get("Userdata", "typ", "Beleglink");
		$AC->addAssocV3("wert", "LIKE", "$HID:%");
		$L = $AC->n();
		if(!$L)
			return "<p>Der zugehörige Beleg konnte nicht gefunden werden.</p>";

		$BID = str_replace("BeleglinkGRLBMID", "", $L->A("name"));
