...
class htmlMimeMail5 {
	...
	/**
	 * The calendar part of the message
	 * @var string
	 */
	private $calendar;
	private $calendarMethod;
	...
	/**
	 * Accessor function to set the calendar charset
	 * 
	 * @param string $charset Character set to use
	 */
	public function setCalendarCharset($charset = 'ISO-8859-1') {
	...
	/**
	 * Adds calendar information as its own
	 * message part
	 * 
	 * @param string $ics ICS content of email
	 */
	public function setCalendar($ics, $method){
		$this->calendar = $ics;
		$this->calendarMethod = $method;
	}
	...
	/**
	 * Adds a calendar subpart to a mime_part object
	 * 
	 * @param  object $obj 
	 * @return object      Mime part object
	 */
	private function addCalendarPart(&$message) {
		$params['content_type'] = 'text/calendar';
		$params['encoding'] = $this->build_params['calendar_encoding']->getType();
		$params['charset'] = $this->build_params['calendar_charset'];
		$params['method'] = $this->calendarMethod;

		if (!empty($message)) {
			$message->addSubpart($this->calendar, $params);
		} else {
			$message = new Mail_mimePart($this->calendar, $params);
		}
	}
	 ...
	 * @return mixed
	 */
	public function send($recipients, $type = 'mail', $forceRebuild = false) {

		if ($this->default_method != null) {
			$type = $this->default_method;
		}
		...
		if (!$this->is_built OR $forceRebuild) {
			$this->build();
		}
...
class fileAttachment extends attachment {
	 ...
	 */
	public function __construct($filename, $contentType = 'application/octet-stream', $encoding = null, $name = '') {
