	...
	if ($PMBP_SYS_VAR['F_updates']) {
		...
		if (!$_SESSION['multi_user_mode'] && !isset($_SESSION['PMBP_VERSION'])) {
			...
			if (strpos($_SERVER['SERVER_SOFTWARE'], "Win"))
				$ping = "ping -n 2 -w 500 phpmybackup.sourceforge.net";
			else
				$ping = "ping -c 2 -W 500 phpmybackup.sourceforge.net";
...
/*function PMBP_email_store($attachments, $backup_info) {
	...
	if (is_array($backup_info)) {
		...
		if ($backup_info['drop'])
			$mailtext.=INF_DROP . ": " . F_YES . $lb;
		else
			$mailtext.=INF_DROP . ": " . F_NO . $lb;
		if ($backup_info['tables'])
			$mailtext.=INF_TABLES . ": " . F_YES . $lb;
		else
			$mailtext.=INF_TABLES . ": " . F_NO . $lb;
		if ($backup_info['data'])
			$mailtext.=INF_DATA . ": " . F_YES . $lb;
		else
			$mailtext.=INF_DATA . ": " . F_NO . $lb;
		...
		if (!$login_result = @ftp_login($conn_id, $CONF['ftp_user'], $CONF['ftp_passwd'])) {
			...
			if (is_array($remote_files)) {
				...
				for ($i = 0; $i < count($remote_files); $i++)
					if (strrchr($remote_files[$i], "/"))
						$remote_files[$i] = substr(strrchr($remote_files[$i], "/"), 1);
...
/*function PMBP_get_dirs($dir, $renew = FALSE) {
	...
	if (isset($_GET['update_dir_list']))
		$renew = true;

	// return existing data
	if ($renew)
		unset($_SESSION['file_system'][$dir]);
	if (isset($_SESSION['file_system'][$dir]))
		return $_SESSION['file_system'][$dir];
