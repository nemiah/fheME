...
//        $MoonPar = $mparallax / $MoonDFrac;                                     // Moon's parallax
	...
	public function getPhaseEmoji(): string
	{
		$emoji = [
			"🌑",
			"🌒",
		...
		return $emoji[floor(($this->phase + 0.0625) * 8)];
	}
	
	public function getPhaseKey(){
        $names = [
            'new_moon',
            'waxing_crescent',
            'waxing_quarter',
	...
	public function getNextFullMoon()
	{
		$current = $this->getPhaseFullMoon();
		if($current < $this->timestamp)
			return $this->getPhaseNextFullMoon ();
		
		return $current;
	}
	
	public function getNextNewMoon()
	{
		$current = $this->getPhaseNewMoon();
		if($current < $this->timestamp)
			return $this->getPhaseNextNewMoon ();
