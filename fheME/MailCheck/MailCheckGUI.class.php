...
class MailCheckGUI extends MailCheck implements iGUIHTML2 {
	...
	public function check($touch = false){
		...
		foreach ($result as $overview) {
			#print_r($overview);
			$T->addRow(array("<small style=\"color:grey;float:right;\">".Util::CLDateParser($overview->udate)."</small>".$overview->from."<br /><small style=\"color:grey;\">".utf8_encode($this->decodeBlubb($overview->subject))."</small>"));
		}
		imap_close($mbox);
		#echo "</pre></div>";
